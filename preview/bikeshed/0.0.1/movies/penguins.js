...
var app = {
  ...
  setupControls: function(){
    var controlLayer = new bikeshed.Group();

    // background
    bikeshed.Shape.rect(0,0,400,35).attr({
      fillColor: 'rgba(0,0,0,0.2)',
      filters: 'blur',
      lineWidth: 1
    }).addTo(controlLayer);

    new bikeshed.Text('Penguins:').attr({
      x: 20,
      y: 10,
      fontSize: 20
    }).addTo(controlLayer);

    this.controls.addButton = new bikeshed.Text('+').attr({
      x: 110,
      y: 0,
      fontSize: 40
    ...
    this.controls.removeButton = new bikeshed.Text('-').attr({
      x: 150,
      y: 0,
      fontSize: 40
    }).addTo(controlLayer).on('pointerdown', function(){
    ...
    new bikeshed.Text('Zoom:').attr({
      x: 200,
      y: 10,
      fontSize: 20
    }).addTo(controlLayer);

    this.controls.zoomInButton = new bikeshed.Text('+').attr({
      x: 270,
      y: 0,
      fontSize: 40
    }).addTo(controlLayer).on('pointerdown', function(){
    ...
    this.controls.zoomOutButton = new bikeshed.Text('-').attr({
      x: 320,
      y: 0,
      fontSize: 40
    }).addTo(controlLayer).on('pointerdown', function(){
  ...
  setupCanvas: function(){
    var canvas = new bikeshed.Group();

    var background = this.background = bikeshed.Shape.rect(0,0, 500, 500).attr({
      x: 0,
      y: 0,
  ...
  addItem: function(){
    var item = new bikeshed.Bitmap('assets/jester_standard.svg', {
      onload: function() {
        this.attr({
          scale: 0.2,
